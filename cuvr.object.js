CuVR.Object = function(root, originalElement) {
  // 各 .cuvr-face にコピー
  var elements = Array.prototype.map.call(root.querySelectorAll('.cuvr-face'), function(face) {
    var cloned = originalElement.cloneNode();
  ...
  // 位置
  var pos = {
    x: Number(originalElement.dataset.posX) || 0,
    y: Number(originalElement.dataset.posY) || 0,
    z: Number(originalElement.dataset.posZ) || -1
  };

  // 向き
  var direction = {
    x: Number(originalElement.dataset.directionX) || 1,
    y: Number(originalElement.dataset.directionY) || 0
  };
